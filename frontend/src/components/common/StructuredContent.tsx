        ...
        if (block.type === "list") {
          return (
            <ul
              ...
              className="space-y-3 text-sm leading-7 text-slate-700"
            >
              {block.items.map((item) => (
                <li key={item} className="flex gap-3 rounded-2xl bg-white/80 px-3 py-2">
...
function renderInline(text: string) {
  ...
  return segments.map((segment, index) => {
    ...
    if (boldMatch) {
      return (
        <strong key={`${boldMatch[1]}-${index}`} className="font-semibold text-slate-950">
          {boldMatch[1]}
        </strong>
      );
    }
